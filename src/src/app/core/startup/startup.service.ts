...
export class StartupService {
   ...
   * @returns 观察对象
   */
  loadLang(): Observable<void> {
    const defaultLang = this.i18n.defaultLang;
    return zip(this.i18n.loadLangData(defaultLang)).pipe(
      // 接收其他拦截器后产生的异常消息
      catchError(res => {
   ...
   * @returns 观察对象
   */
  loadAppData(url?: string): Observable<void> {
    const appDataUrl = url ? url : '/assets/json/app-data.json';
    return zip(this.httpClient.get(appDataUrl)).pipe(
      // 接收其他拦截器后产生的异常消息
      catchError(res => {
        console.warn(`StartupService.load: Network request failed`, res);
        setTimeout(() => this.router.navigateByUrl(`/exception/500`));
